...
use constant DEVICE_DISPLAY_NAME => 'LCD 20x4 Bricklet';
...
=item FUNCTION_SET_DEFAULT_TEXT
...
=item FUNCTION_GET_DEFAULT_TEXT
...
=item FUNCTION_SET_DEFAULT_TEXT_COUNTER
...
=item FUNCTION_GET_DEFAULT_TEXT_COUNTER
...
sub new
{
	...
	$self->{response_expected}->{&FUNCTION_SET_DEFAULT_TEXT} = Tinkerforge::Device->_RESPONSE_EXPECTED_FALSE;
	$self->{response_expected}->{&FUNCTION_GET_DEFAULT_TEXT} = Tinkerforge::Device->_RESPONSE_EXPECTED_ALWAYS_TRUE;
	$self->{response_expected}->{&FUNCTION_SET_DEFAULT_TEXT_COUNTER} = Tinkerforge::Device->_RESPONSE_EXPECTED_FALSE;
	$self->{response_expected}->{&FUNCTION_GET_DEFAULT_TEXT_COUNTER} = Tinkerforge::Device->_RESPONSE_EXPECTED_ALWAYS_TRUE;
...
sub write_line
{
	...
	$self->_send_request(&FUNCTION_WRITE_LINE, [$line, $position, $text], 'C C Z20', '');
}
...
sub set_default_text
{
	...
	$self->_send_request(&FUNCTION_SET_DEFAULT_TEXT, [$line, $text], 'C Z20', '');
}

=item get_default_text()
...
sub get_default_text
{
	...
	return $self->_send_request(&FUNCTION_GET_DEFAULT_TEXT, [$line], 'C', 'Z20');
}

=item set_default_text_counter()
...
sub set_default_text_counter
{
	...
	$self->_send_request(&FUNCTION_SET_DEFAULT_TEXT_COUNTER, [$counter], 'l', '');
}

=item get_default_text_counter()
...
sub get_default_text_counter
{
	...
	return $self->_send_request(&FUNCTION_GET_DEFAULT_TEXT_COUNTER, [], '', 'l');
}
